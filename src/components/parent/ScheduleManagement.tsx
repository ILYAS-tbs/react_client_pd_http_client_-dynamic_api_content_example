...
import {
  ScheduleManagementParentProps,
  ScheduleManagementProps,
} from "../../types";
import { ClassGroup } from "../../models/ClassGroups";
import { handleDownloadFromURL } from "../../lib/download_script _from_url";
...
const ScheduleManagement: React.FC<ScheduleManagementParentProps> = ({
  ...
  function mapClassGroupToSchedule(
    ...
    return class_groups_list.map((classGroup: ClassGroup) => ({
      ...
      // uploadedAt: classGroup.,
      childName:
        students.find(
          (student) =>
            student.class_group?.class_group_id == classGroup.class_group_id
        )?.full_name ?? "",
    }));
  }
  ...
  >({ childName: "", className: "", pdfUrl: null });
  ...
  const handleFileUpload = (file: File) => {
    ...
    reader.onload = () => {
      ...
      if (editingSchedule) {
        ...
        setSchedules(
          schedules.map((sch) =>
            sch.id === editingSchedule.id
              ? { ...sch, pdfUrl: base64String, uploadedAt }
              : sch
          )
        );
      } else {
        ...
        setSchedules([
          ...schedules,
          { ...newSchedule, id: newId, pdfUrl: base64String },
        ]);
      }
  ...
  const handleEditAndDownloadSchedule = (schedule: ScheduleItem) => {
    ...
    //? DOWNLOAD IT
    handleDownloadFromURL(schedule.pdfUrl || "");
  };
